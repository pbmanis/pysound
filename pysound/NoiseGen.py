...
class Controller(object):
    ...
    def start_run(self):
       ...
       # self.showParameters()
        ...
        self.TrialTimer.start(1) # start (almost) right away
    ...
    def continue_run(self):
        ...
        if self.pause_hit:
            ...
            if self.trial_active:
                self.TrialTimer.start(1) # start (almost) right away where we left off
        else:
            return
        ...
        #TFR, changing Noise Search to possibly present a train of noise bursts 20180129
            ...
            if self.CPars['Noise Train']['Number']>1:
                ...
                wave = sound.NoisePip(rate=Fs, duration=self.CPars['Stimulus']['Duration']+self.CPars['Stimulus']['Delay']+0,
                            ...
                            pip_starst=1e-3*np.arange(self.CPars['Stimulus']['Delay']*1000.,howmany,self.CPars['Noise Train']['Interval']),
